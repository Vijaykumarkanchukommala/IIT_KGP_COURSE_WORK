...
(
   input    [SAMPLE_WIDTH - 1:0]     i_A [NUM_SAMPLES-1:0], 
   input    [SAMPLE_WIDTH - 1:0]     i_B [NUM_SAMPLES-1:0],
   input                             i_load               ,
  ...
  wire  [MULT_RESULT_SAMPLE_WIDTH-1:0] w_multiplier_result[NUM_SAMPLES-1:0];
  ...
  generate for(stg_i = 0; stg_i < NUM_ADDER_STAGES; stg_i = stg_i + 1) begin :gen_stg
    wire [(MULT_RESULT_SAMPLE_WIDTH+stg_i)-1:0]     w_addr_in_a  [(NUM_EVEN_SAMPLES/(2**(stg_i+1)))-1:0];
    wire [(MULT_RESULT_SAMPLE_WIDTH+stg_i)-1:0]     w_addr_in_b  [(NUM_EVEN_SAMPLES/(2**(stg_i+1)))-1:0];
    wire [(MULT_RESULT_SAMPLE_WIDTH+stg_i+1)-1:0]   w_result   [(NUM_EVEN_SAMPLES/(2**(stg_i+1)))-1:0];
  ...
  generate 
   if((NUM_SAMPLES%2)!=0) begin  :final_odd_addr_stg
      if(ADDER_TYPE == 0) begin  
         ...
         (
            .i_A                   ({{NUM_ADDER_STAGES{1'b0}},w_multiplier_result[NUM_SAMPLES-1]}),
